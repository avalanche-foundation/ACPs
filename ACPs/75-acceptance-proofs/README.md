...
Subnets are able to prove arbitrary events using warp messaging but a common pattern that would be useful to natively support would be proving block acceptance to other subnets. One example use case for acceptance proofs is to provide stronger fault isolation guarantees from the primary network to subnets.
